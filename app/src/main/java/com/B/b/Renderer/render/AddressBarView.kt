 ...
 * 詳細な経緯はDebugDrawerView.ktの同名ヘルパーのコメント参照。このアドレスバーは
 * 元々「ステータスバーと被らない・視認的におかしくない」を前提にdpベースで
 * 高さ・余白を調整済みだったため(=拡大縮小設定の影響を受けないのが「あるべき100%」)、
 * テキストサイズもそれに合わせてfontScaleを無視する。
 */
private fun android.widget.TextView.setFixedTextSize(spValue: Float) {
